...
const program = new Command('doctor')
    ...
    try {
        ...
        uvOk = true;
        if (!jsonOutput)
            printSuccess('uv (Python package manager) found');
    }
    catch {
        warnings.push('uv not found (needed for MCP registry)');
        if (!jsonOutput)
            printWarning('uv not found → recommended for MCP registry');
    }
    results.uv = { ok: uvOk };
    ...
    portResults.forEach((p, i) => {
        ...
        if (p.isInUse) {
            ...
            if (!jsonOutput)
                printSuccess(`Port ${p.port} (${names[i]}) is free`);
        }
    });
    ...
    if (existsSync(catalogPath)) {
        try {
            ...
            catalogOk = true;
            if (!jsonOutput)
                printSuccess(`catalog.json is valid (${catalogPath})`);
        }
        catch {
            if (!jsonOutput)
                printError('catalog.json exists but is invalid JSON');
            issues.push('Invalid catalog.json');
        }
    }
    else {
        if (!jsonOutput)
            printWarning(`catalog.json not found at ${catalogPath}`);
        warnings.push('catalog.json missing');
        if (repoSource === 'cwd') {
            warnings.push('Could not locate server/data/catalog.json — run from the monorepo root or set FORGE_REPO_ROOT.');
        }
    }
    ...
    if (options.deep) {
        ...
        const smokeHttpBase = process.env.FORGE_SMOKE_HTTP_BASE?.trim() ||
    ...
    if (proxyOn && !allowOn) {
        warnings.push('FORGE_API_PROXY is set without FORGE_ALLOW_PROXY=1 — Vite uses embedded catalog (proxy ignored). Add FORGE_ALLOW_PROXY=1 only if you intentionally proxy /api to 5181.');
        if (!jsonOutput) {
    ...
    if (!options.quick && webPortInfo?.isInUse) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (!jsonOutput)
                    printSuccess(`Catalog API reachable (${n} packs) on port ${cfg.ports.web}`);
            }
            else {
                results.catalogHttp = { ok: false, url: catalogUrl, status: res.status };
                warnings.push(`Catalog HTTP ${res.status} on port ${cfg.ports.web}`);
                if (!jsonOutput)
                    printWarning(`Catalog API returned HTTP ${res.status} — check FORGE_API_PROXY and API on 5181.`);
            }
        }
        catch (e) {
            ...
            if (!jsonOutput)
                printWarning(`Could not fetch catalog on :${cfg.ports.web} — is Vite running?`);
        }
    }
    ...
    if (!options.quick && proxyOn && allowOn) {
        ...
        if (apiUnhealthy) {
            const w = 'Proxy mode (FORGE_API_PROXY + FORGE_ALLOW_PROXY): standalone API on 5181 is required but not healthy. Start `npm run dev:api`, use `npm run dev:wait-api` to start web after /ready, or unset FORGE_ALLOW_PROXY to use Vite embedded catalog.';
            warnings.push(w);
            if (!jsonOutput)
                printWarning(w);
        }
    ...
    if (jsonOutput) {
        const response = createCiResponse(exitCode === ExitCodes.SUCCESS, exitCode, {
            healthy: isHealthy,
            checks: results,
            summary: {
