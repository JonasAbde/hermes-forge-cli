...
import { readFile, readdir, mkdir, writeFile, copyFile } from 'fs/promises';
import { join } from 'path';
import chalk from 'chalk';
import ora from 'ora';
import { printHeader, printSuccess, printError, printInfo, printWarning } from '../../lib/output.js';
async function findPacks(packId) {
...
async function getPackInfo(id, path) {
...
async function buildMetadata(pack, outputDir) {
...
async function buildCutouts(pack, outputDir) {
    ...
    const files = await readdir(cutoutsDir);
    const imageFiles = files.filter(f => f.endsWith('.png') || f.endsWith('.jpg') || f.endsWith('.jpeg') || f.endsWith('.svg'));
    let copied = 0;
...
async function buildPack(pack, options) {
    const errors = [];
    try {
...
const program = new Command('build')
    ...
    try {
        ...
        for (const pack of packs) {
            const buildTargets = [];
            if (!options.cutoutsOnly && pack.hasMetadata)
                buildTargets.push('metadata');
            if (!options.metadataOnly && pack.hasCutouts)
                buildTargets.push('cutouts');
