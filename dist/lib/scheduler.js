...
function matchesSchedule(date, minute, hour, dayOfMonth, month, dayOfWeek) {
    ...
    return (matches(date.getMinutes(), minute) &&
...
export async function getTaskLogs(taskId, limit = 10) {
    try {
...
// Clean old logs
export async function cleanOldLogs(olderThanDays = 7) {
    try {
        const files = await readdir(LOGS_DIR);
