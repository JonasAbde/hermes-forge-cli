  ...
  .addCommand(
    new Command('get')
      ...
      .action((key: string | undefined, options) => {
        const cfg = config.get() as unknown as Record<string, unknown>;
  ...
  .addCommand(
    new Command('set')
      ...
      .action((key: string, rawValue: string) => {
        ...
        if (!key.includes('.')) {
          const topKey = key as keyof CliConfig;
          try {
            config.set(topKey, value as CliConfig[typeof topKey]);
          } catch (err: unknown) {
            printError(`Failed to set ${key}: ${(err as Error).message}`);
            process.exit(1);
          }
        } else {
          // Nested dot-path: read full config, update in-place, write back top-level key
          const cfg = config.get() as unknown as Record<string, unknown>;
          const topKey = key.split('.')[0] as keyof CliConfig;
          ...
          const updated = JSON.parse(JSON.stringify(topValue)) as Record<string, unknown>;
          const subPath = key.split('.').slice(1).join('.');
          setNestedValue(updated, subPath, value);
          try {
            config.set(topKey, updated as CliConfig[typeof topKey]);
          } catch (err: unknown) {
            printError(`Failed to set ${key}: ${(err as Error).message}`);
            process.exit(1);
          }
    ...
    new Command('reset')
      ...
      .action(async (options) => {
        ...
        try {
          ...
          (config as unknown as { config: { clear(): void } }).config.clear();
...
function getConfigFilePath(): string {
  try {
    return (config as unknown as { config: { path: string } }).config.path;
  } catch {
