...
const program = new Command('doctor')
  ...
  .action(async (options) => {
    ...
    if (proxyOn && !allowOn) {
      warnings.push(
        'FORGE_API_PROXY is set without FORGE_ALLOW_PROXY=1 — Vite uses embedded catalog (proxy ignored).'
      );
      if (!jsonOutput) {
        printWarning(
    ...
    if (!options.quick && proxyOn && allowOn) {
      ...
      if (apiUnhealthy) {
        const w =
          'Proxy mode (FORGE_API_PROXY + FORGE_ALLOW_PROXY): standalone API on 5181 is required but not healthy. Start `npm run dev:api`, use `npm run dev:wait-api` to start web after the API is ready, or unset FORGE_ALLOW_PROXY to use Vite embedded catalog.';
        warnings.push(w);
