...
import { detectEnvironment, type EnvInfo } from '../lib/envDetector.js';

// ── Systemd unit map ────────────────────────────────────────────
// Maps forge service names to their systemd unit names.
const SYSTEMD_UNIT_MAP: Record<string, string> = {
 ...
 */
async function listSystemdUnits(env: EnvInfo): Promise<Array<{ service: string; unit: string; active: string }>> {
  if (!env.hasSystemd) return [];

  const results: Array<{ service: string; unit: string; active: string }> = [];
 ...
 */
async function listRunningForgeUnits(env: EnvInfo): Promise<string[]> {
  if (!env.hasSystemd) return [];
 ...
 */
async function journalctlLogs(unit: string, lines: number): Promise<string> {
 ...
 */
function followJournalctl(unit: string, onLine: (line: string) => void): () => void {
...
function matchesLevel(entry: LogEntry, levelFilter?: string): boolean {
