...
export async function terminateProcess(pid: number, graceful = true): Promise<void> {
  try {
...
export async function findProcessesByPort(port: number): Promise<ProcessByPort[]> {
  const processes: ProcessByPort[] = [];
