...
export async function getWorkspace(idOrName: string): Promise<Workspace | null> {
  const workspaces = await loadWorkspaces();
...
export async function setDefaultWorkspace(id: string): Promise<boolean> {
  const workspaces = await loadWorkspaces();
...
// Switch to workspace
export async function switchToWorkspace(idOrName: string): Promise<Workspace | null> {
...
export async function detectWorkspaces(searchPaths: string[] = []): Promise<Array<{
  path: string;
  name: string;
  ...
  const detected: Array<{ path: string; name: string; confidence: 'high' | 'medium' | 'low' }> = [];
...
export async function importCurrentDirectory(): Promise<Workspace | null> {
...
export async function exportWorkspaceConfig(workspaceId: string): Promise<string | null> {
  const workspace = await getWorkspace(workspaceId);
  if (!workspace) return null;
...
export async function importWorkspaceConfig(configJson: string): Promise<Workspace> {
